...
model_path = os.path.join(os.path.dirname(__file__), 'heart_failure_prediction_model.pkl')
...
def single_prediction():
    ...
    with st.form(key='prediction_form'):
        ...
        submit_button = st.form_submit_button('Predict :stethoscope:', type='secondary')
...
def batch_prediction():
    ...
    if uploaded_file:
        ...
        if st.button("예측 수행 :stethoscope:"):
            ...
            st.download_button(':arrow_down: 예측 결과 다운로드', data=csv, file_name=uploaded_file.name[:-4]+'_predictions.csv', mime='text/csv')
